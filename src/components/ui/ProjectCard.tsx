...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <TiltCard className="h-full">
      <div className="group relative flex flex-col h-full overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-[#15171E] hover:border-[#E58A2B]/60 transition-all duration-500 hover:-translate-y-1 shadow-2xl shadow-black/80">
        {/* Card Media Banner */}
        <div className="relative aspect-[16/9] w-full overflow-hidden bg-[#0B0C10]">
          <Image
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
            <span className="px-2.5 py-0.5 rounded-full bg-black/70 border border-white/10 text-[#E58A2B] font-mono text-[10px] font-bold uppercase tracking-wider backdrop-blur-md">
              {project.category}
            </span>
            <span className="px-2.5 py-0.5 rounded-full bg-black/70 border border-white/10 text-white font-mono text-[10px] font-bold backdrop-blur-md">
              0{index + 1}
            </span>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-start justify-between gap-3">
              <Link href={`/project/${project.id}`} className="group-hover:text-[#E58A2B] transition-colors">
                <h3 className="font-display text-lg sm:text-xl font-bold text-white leading-tight tracking-tight">
                  {project.name}
                </h3>
              </Link>
              <Link
                ...
                className="p-1.5 rounded-full bg-white/5 group-hover:bg-[#E58A2B] text-gray-400 group-hover:text-black transition-all shrink-0"
              >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {project.skillsUsed.slice(0, 3).map((skill) => (
                <span
                  key={skill}
                  className="rounded-full border border-white/10 px-2.5 py-0.5 text-[10px] sm:text-[11px] font-mono text-gray-300 bg-white/5"
                >
                  {skill}
                </span>
              ))}
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/10 font-mono text-xs">
              <Link
                href={`/project/${project.id}`}
                className="font-bold text-[#E58A2B] hover:underline flex items-center gap-1 text-[11px] sm:text-xs"
              >
